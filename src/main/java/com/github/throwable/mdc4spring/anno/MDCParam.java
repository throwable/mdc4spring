...
/**
 * Defines a parameter that will be added to current MDC. The parameter will be present in all logging traces
 ...
 * during the method invocation. By default, the parameter will have the argument's name and the value the method invoked with.
 ...
 *     <li>By default Java compiler does not keep method argument names in generated bytecode, so it will cause
 *     possible problems with parameter name resolutions. There are three options to solve this problem:
 *     <ul>
 *          <li>If you are using Spring Boot and Spring Boot Maven or Gradle plugin all method arguments will be already kept
 *          with their names, and no additional action is required.</li>
 *          <li>If you are not using Spring Boot plugin you may tell to the compiler to preserve method argument names
 *          by adding <code>-parameters</code> argument to javac invocation.</li>
 *          <li>You may also define parameter name for every argument explicitly:
